...
UVCControl.prototype.init = function() {
  var devices = usb.getDeviceList();
    var matchedDevices = devices.filter(device => 
        device.deviceDescriptor.idVendor === this.vid &&
        device.deviceDescriptor.idProduct === this.pid &&
        device.serialNumber === this.uniqueIdentifier // Assuming you can get serialNumber or similar
    );

    if (matchedDevices.length > 0) {
        this.device = matchedDevices[this.options.camNum]; // Assuming the first matched device is the one you want
        this.device.open();
