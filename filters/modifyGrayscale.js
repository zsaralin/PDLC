...
export function modifyGrayscale(canvas) {
    ...
    for (let i = 0; i < imageData.data.length; i += 4) {
        ...
        if (grayExpo.value === '1') { // Ensure comparison as string if necessary
            ...
            if (average < sliderLowValue) {
                average = minPixelValue + Math.pow((average / sliderLowValue), parseFloat(grayExpo.value)) * RANGE;
            } else if (average > sliderHighValue) {
                average = sliderHighValue + Math.pow((average / maxPixelValue), parseFloat(grayExpo.value)) * RANGE;
            }
